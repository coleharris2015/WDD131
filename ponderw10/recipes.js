...
function tagsTemplate(tags) {
    return tags.map(tag => `<li><button class="tag-btn" type="button">${tag}</button></li>`).join("");
}
...
function recipeTemplate(recipe) {
    ...
    const instructionsList = recipe.recipeInstructions.map((step, index) => `<li>**Step ${index + 1}:** ${step}</li>`).join('');

    return `
        <div class="recipe" data-recipe-name="${recipe.name.toLowerCase()}">
            ...
            <div class="summary-content">
                ...
                <div class="rating">${ratingTemplate(recipe.rating)}</div>
            </div>
            
            <div class="details-content">
                <div class="detail-section">
                    <h3>Ingredients (${recipe.recipeYield || 'Unknown Yield'})</h3>
                ...
                <div class="metadata">
                    <span>⏲️ Prep: ${recipe.prepTime} | ⏱️ Cook: ${recipe.cookTime}</span>
                    <span>✍️ Author: ${recipe.author}</span>
                </div>
            </div>
        </div>
    `;
...
function renderRecipes(recipesToRender) {
    const container = document.getElementById("renderedArea");
    if (recipesToRender.length === 0) {
        container.innerHTML = "<p style='text-align: center; font-size: 1.5em;'>No recipes found.</p>";
        return;
    }
    const sortedRecipes = recipesToRender.sort((a, b) =>
        a.name.toLowerCase() > b.name.toLowerCase() ? 1 : -1
    );
    container.innerHTML = sortedRecipes.map(recipeTemplate).join("");
}
